# Design.MD — Curium Design System
...
본 문서는 _Curium_ 의 design system 의 _single source of truth_. PRD/DEVPLAN 은 _무엇을 / 어떻게 / 언제_, Design.MD 는 _어떻게 보이고 어떻게 느껴지는가_.
...
- _Figma file_ (Apple Design Resources iOS 26 baseline + Curium 독립) 과 _이중 sync_ — Figma 가 _Visual single source_, Design.MD 가 _Decision + 근거 single source_
...
| _White background + clean layout_ | 단 Curium 는 _warm white_ (parchment) — _차가운 흰색_ 보다 _따뜻한 종이_ 톤 |
| _Avenir typography_ — 부드러운 sans-serif | Curium 는 _System SF Pro_ (Apple-native, 8 locale fallback) + _New York serif_ (Item 제목, _slow + library_ 톤) |
| _Time-blocking 일일 의례_ | Curium 의 _Time Travel_ (anniversary / "On this day") = _시간 의례_ 의 collection 버전 |

**Sunsama 와 Curium 의 _다른_ 점**:
...
→ Sunsama 정신은 _"calm + intentional"_ 핵심. Curium 의 _"느린 큐레이션"_ 과 합성.
...
| _Apple ecosystem exclusive_ — Mac / iPhone / iPad / Watch / Vision Pro | Curium 도 _Apple-native first_, iOS 26 only deployment |
| _Quick Entry_ — keyboard shortcut 으로 즉시 task 추가 | Curium 의 _3-second capture_ (바코드 스캔 + 음성 메모) + Quick Capture 위젯 (v1.x) |
| _Today / Upcoming / Anytime / Someday_ 단순 카테고리 | Curium 의 _Library / Collections / Time Travel_ 3 카테고리 (no algorithm) |

**Things 3 와 Curium 의 _다른_ 점**:
...
→ Things 3 정신은 _"minimal + elegant + power tucked away"_ 핵심. Curium 의 _Item-as-memory-anchor_ 와 합성.

### 1.2 합성된 Curium Design 정신 (4 차원)
...
→ **Curium = _Slow library of memory_** — Things 3 의 _craftsmanship_ + Sunsama 의 _calm intentionality_ + Item-as-memory-anchor 데이터 모델.
...
- _Block + Channel + M:N connection_ → Curium _Item + Collection + M:N_ 매핑
- _재귀 Channel nesting_ → v1.x+ 검토 (Pre-flight v1.0 단순 유지)
- _Editorial layer_ (Are.na editorial) → Curium 의 _Settings "Curium is" surface + Founder voice_

**회피**:
- _Public-default channel_ — Curium default = private
- _Web-first_ — Curium = Apple-native first

### 2.2 Day One — Apple ecosystem 깊이 통합

**차용**:
- _Watch / Shortcuts / Health / Siri_ integration → Curium v2.0+
- _Encrypted entries_ → CloudKit Private DB (자동 encryption)
- _"On this day"_ 회고 → Curium 의 _Time Travel anniversary_
- _Entry photo + voice + drawing_ attachments → Curium 의 _Memory.photos / .voiceNote_

**회피**:
- _Entry-anchored_ 데이터 모델 — Curium 의 _Item-anchored_ 역방향
- _Daily Chat_ AI (Gold tier 2026) — Promise §5.4 (No AI judgment)
- _Subscription model_ — Curium 의 paid one-time

### 2.3 Letterboxd — Lists 의도된 큐레이션

**차용**:
- _Lists_ 기능 (사용자가 만든 의미 단위 묶음) → Curium _Collection_
- _Diary_ 시간 기록 → Curium _Memory.kind = watched_
- _Year in Review_ 연말 회고 → Curium v1.x _1주년 Letter_ (T5 정합)
- _Beautiful design_ — film posters 의 _visual treatment_

**회피**:
- _Star rating_ 강요 → Curium default OFF (별점 자체 없음, T2 Tier)
- _Social network_ (follow / like / comment) — Promise §5.2 (No feed)

### 2.4 Discogs — Work/Release 데이터 모델 시각화

**차용**:
- _Master/Release_ 패턴 → Curium _Item / Manifestation_ 1:N
- _External identifiers_ (catalog number, ISBN, MusicBrainz ID) 정규화
- _깊은 typed metadata_ (Format / Country / Label) → Curium Music typed metadata

**회피**:
- _Marketplace_ 기능 — Forever §5.3 약속 위반 가능
- _UX dated_ — Curium 는 modern minimal

### 2.5 Plumerie — Lending tracker first-class

**차용**:
- _대출 / 선물 / 받음_ 흐름 first-class → Curium _Loan_ entity
- _Overdue flag_ → Curium v1.x 알림 (default OFF, opt-in)
- _Family sharing_ → Curium 는 _v2.0+ 검토_ (Forever 정합 검토 후)

**회피**:
- _Web-first_ — Curium Apple-native
- _책 전용_ — Curium 4 medium

### 2.6 Tot — Apple-native paid one-time premium quality

**차용**:
- _$20 paid one-time_ premium positioning → Curium $14.99 (Tot 가격대 정합)
- _Apple-native_ + _system font (SF Pro)_ 충실
- _Minimum viable UI_ — 7 슬롯만 (deep simplicity)

**회피**:
- _Limited 7 슬롯_ — Curium 는 unlimited items
- _노트 only_ — Curium 다중 medium

### 2.7 1SE — 시간 단위 archive

**차용**:
- _시간 단위 mosaic_ → Curium Time Travel 시각화 (5년 전 오늘 / 시기별)
- _Anniversary 알림_ → Curium v1.x

**회피**:
- _시간 axis only_ — Curium _Item axis_ 가 primary
- _Subscription_ — Curium paid one-time

### 2.8 Polarsteps — 자동 GPS 추적

**차용**:
- _CoreLocation 자동_ 위치 메타데이터 → Curium Capture flow (위치 자동 제안)
- _인쇄 책자_ (PDF export 정교화) → Curium v2.0+ 검토
- _Trip Reels_ — Curium Time Travel 시각화 영감

**회피**:
- _여행 only_ — Curium 다중 medium

### 2.9 Sunsama — Calm + Intentional

**차용**:
- _Calm + muted_ 디자인 톤 → Curium palette warm parchment
- _Mindful planning_ — 매 capture / share 가 _의식적_
- _Avenir 영감_ → Curium 의 _SF Pro_ (Avenir 와 _humanist sans_ 정합)

**회피**:
- _Daily planner_ 기능 — Curium 는 collection
...
- _Quick Entry_ — Curium _3-second capture_ 영감
- _Today/Upcoming/Anytime/Someday_ 단순 카테고리 → Curium _Library / Collections / Time Travel_

### 2.11 Curium _차용 안 하는_ 항목 (anti-pattern)

| Anti-pattern | 출처 | 이유 |
|----------|-----|----|
| Public social profile / follow | Goodreads / Letterboxd / Are.na | Promise §5.2 |
| 별점 강요 | Goodreads / Letterboxd | Curium Tier 2 (별점 자체 없음) |
...
| Marketplace 통합 | Discogs / Kolekto | Curium 정체성 |
...
| Token | Hex | 용도 |
|-------|-----|----|
| `color.background` | `#F5F0E8` | Warm parchment (Curium 의 정체성 색) — _차가운 흰색 회피_, _도서관의 종이_ 톤 |
...
|-------|------|--------|--------------------------|----|
| `font.display` | New York | Medium | 28pt (Large Title) | Curium 명함 / Settings header |
...
### 5.6 Settings → "Curium is" surface
...
| Settings → "Curium is" header | Semi-translucent material |

### 8.3 Curium Palette + Liquid Glass 합성
...
§3.2 참조. `CuriumTypeface` helper namespace 로 _locale 기반 자동 fallback_.
...
- Visual continuity: _Curium 의 burnt sienna accent_ + _시스템 폰트_ 사용한 cover image
...
3. _Curium 정신 정합_ (Time Travel = 과거 회고 → _chronological 정합_, 단 default 진입은 _최근 entry_ 보이게)
4. _결정 + Design.MD 섹션 추가 + Figma_

### 12.3 Figma File 연계

- Apple Design Resources iOS 26 Figma = _baseline_
- Curium Figma file = _독립_ workspace (Apple baseline 위 _독립 design tokens_ 와 _component library_)
...
- Editorial: §11 Settings "Curium is" + Founder voice
...
- [ ] Settings → "Curium is" 카피 ko/en 1차 draft
